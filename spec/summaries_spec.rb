...
describe Summaries::Daily::Trends do
  subject { described_class.new(cumulative: summary_report, detailed: detailed_report) }
  ...
  describe "#build_trends_for" do
    it "should generate a trend data row based on Time Log data for the day" do
      expect(subject.build(date: friday)).to eq(trends)
    end
    ...
    it "should return an empty trends row if there was no data on the date given" do
      expect(subject.build(date: before_logging)).to eq(empty_trends)
    end
  end
end
